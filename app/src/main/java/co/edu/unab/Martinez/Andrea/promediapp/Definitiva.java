...
public class Definitiva implements Serializable {
    ...
    public double getDefinitiva(){
        ...
        double definitiva = this.getPrimerCorte()*0.5 + this.getSegundoCorte()*0.5;
        return definitiva;

    }

    public double getPrimerCorte(){
        double uno = this.asistencia1*0.1 + this.trabajos1*0.3 + this.talleres*0.3 + this.parcial*0.3;
        return uno;
    }

    public double getSegundoCorte(){
        double dos = this.asistencia2*0.1 + this.trabajos2*0.3 + this.entregable1*0.15 + this.entregable2*0.15 + this.sustentacion*0.15 + this.aplicacion*0.15;
        return dos;
    }
